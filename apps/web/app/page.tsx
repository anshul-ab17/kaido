...
function MarketStatsBar() {
  ...
  const sol = tickers['SOL-PERP'];
  const price = sol?.price ?? 145.20;

  const stats = [
    { label: 'SOL-PERP', value: `$${price.toFixed(2)}`, sub: '+2.45%', positive: true },
    { label: 'Index Price', value: `$${(price - 0.02).toFixed(2)}`, sub: null, positive: true },
                ...
                <div className="glass-card flex flex-col min-h-[280px]">
                  <div className="flex border-b border-white/5">
                    {['Positions (0)', 'Open Orders', 'Trade History', 'Fills'].map((tab, i) => (
                      <button
                        key={tab}
                        className={`px-5 py-3 text-xs font-bold transition-colors ${
                          i === 0
